...
onMounted(async () => {
  ...
  // Fetch preload path from main process. Must complete before the webview
  // element is rendered, otherwise its `preload` attribute is empty.
  preloadPath.value = await window.ada.paths.getWebviewPreload()

  // v-if on <webview> means it renders only AFTER preloadPath is set.
  // Wait for Vue to flush that render before grabbing the ref, otherwise
  // webviewRef.value is still null and we'd silently return.
  await nextTick()
  ...
  const wv = webviewRef.value as
    ...
    | null
  if (!wv) {
    console.warn('[PhoneView] webview ref still null after nextTick')
    return
  }
  ...
  // Receive events from the webview-preload bridge.
  wv.addEventListener('ipc-message', (e: Event) => {
    ...
    if (msg.channel === 'webclient-event' && Array.isArray(msg.args)) {
      const event = msg.args[0] as WebClientEvent
      webclient.ingest(event)
    }
  })
})
...
<template>
  <div class="flex h-full flex-col">
    ...
    >
      ...
      <span class="font-mono text-slate-700">{{ webclientUrl }}</span>
      ...
      >
        ...
        <template v-if="webclient.lastCaller">
          · {{ webclient.lastCaller }}
        </template>
      </span>
      <span class="flex-1" />
      <button
